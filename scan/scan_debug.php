 ...
 * @ Modified time: 2021-03-21 17:45:05
 */
if (!defined('ABSPATH'))
...
//$q = $wpdb->get_var("SELECT count(*) FROM `$table_name`");
$q = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM %i", $table_name));
...
echo '<h3>'. esc_attr__("Scan Debug","antihacker"). '</h3>';
...
$r = $wpdb->get_var($wpdb->prepare("SELECT debug FROM %i ORDER BY id DESC LIMIT 1", $table_name));
  // var_dump($r);


if (empty($r))
   echo esc_attr__("No Debug found...","antihacker");
else {
 ...
 // $r2 = $wpdb->get_var("SELECT date_end from `$table_name` ORDER BY id DESC limit 1");
  $r2 = $wpdb->get_var($wpdb->prepare("SELECT date_end FROM %i ORDER BY id DESC LIMIT 1", $table_name));
